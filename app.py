...
TIER1_WORDS: list[str] = MODERATION_CONFIG["categories"]["tier1_severe_violations"][
    "words"
]
...
def recommend(user_id: int, filter_following: bool):
    ...
    liked_posts: list = list(
        map(
            dict,
            query_db(
                "SELECT posts.id, posts.content FROM posts JOIN reactions ON posts.id = reactions.post_id WHERE reactions.user_id = ?",
                (user_id,),  # type: ignore
            ),
        )
    )
    followed_users = set(
        map(
            lambda u: u["id"],
            query_db(
                "SELECT users.id FROM users JOIN follows ON users.id = follows.followed_id WHERE follows.follower_id = ?",
                (user_id,),
            ),  # type: ignore
        )
    )
    ...
    for post in liked_posts:
        liked_hashtags.update(extract_hashtags(post["content"]))
        liked_posts_ids.add(post["id"])

    res = []
    for post in posts:  # type: ignore
        is_from_followed_author = post["user_id"] in followed_users

        # User has already saw the post
        if post["id"] in liked_posts_ids:
            continue
        if filter_following and not is_from_followed_author:
            continue

        post_hashtags = extract_hashtags(post["content"])
        # Count score as number of overlapping hashtags
        score = len(liked_hashtags.intersection(post_hashtags))
        score += 1 if is_from_followed_author else 0

        if score > 0:
            res.append((score, post["created_at"], post))

    res.sort(key=lambda x: (-x[0], -x[1].timestamp()))
    res = [post for _, _, post in res][:5]

    return res


# Task 3.2
def user_risk_analysis(user_id: int):
    """
    Args:
    ...
    score = 0
    user: dict = dict(
        query_db("SELECT * FROM users WHERE id = ?", (user_id,), one=True)  # type: ignore
    )
    posts: list = query_db("SELECT * FROM posts WHERE user_id = ?", (user_id,))  # type: ignore
    comments: list = query_db("SELECT * FROM comments WHERE user_id = ?", (user_id,))  # type: ignore

    # Step 1: Profile score
    _, profile_score = moderate_content(user.get("profile", ""))

    # Step 2: Post score
    if posts and isinstance(posts, list):
        average_post_score = sum(
            moderate_content(dict(p).get("content", ""))[1] for p in posts
        ) / len(posts)
    else:
    ...
    if comments and isinstance(comments, list):
        average_comment_score = sum(
            moderate_content(dict(c).get("content", ""))[1] for c in comments
        ) / len(comments)
    else:
    ...
    content_risk_score = (
        (profile_score * 1) + (average_post_score * 3) + (average_comment_score * 1)
    )
    ...
    # Step 6: Apply empty profile multiplier
    if not user.get("profile") and (len(posts) < 3 or len(comments) < 3):
        score = content_risk_score * 1.1
...
def moderate_content(content: str):
    ...
    moderated_content_length = len(moderated_content)
    start = 0

    if any(word in lower_content for word in TIER1_WORDS):
        return "[content removed due to severe violation]", 5

    if any(phrase in lower_content for phrase in TIER2_PHRASES):
        return "[content removed due to spam/scam policy]", 5

    while start < moderated_content_length:
        if moderated_content[start].isspace():
            start += 1
            continue

        end = start
        while end < moderated_content_length and not moderated_content[end].isspace():
            end += 1

        word = moderated_content[start:end].lower()
        word_length = len(word)

        if word in TIER3_WORDS:
            score += 2
            moderated_content = (
                moderated_content[:start]
                + ("*" * word_length)
                + moderated_content[end:]
            )

        if validators.url(word) or validators.url("http://" + word):
            score += 2
            moderated_content = (
                moderated_content[:start] + link_replacement + moderated_content[end:]
            )
            end = start + len(link_replacement)
    ...
    if content_length > 15:
        ...
        if uppercase_count / content_length > 0.7:
            score += 0.5
